...
auto UpdateExecutor::Next([[maybe_unused]] Tuple *tuple, RID *rid) -> bool { 
  ...
  while (child_executor_->Next(&old_tuple, rid)) {
    ...
    auto new_rid_optional = table_info_->table_->InsertTuple(TupleMeta{0, false}, new_tuple, ctx->GetLockManager(), nullptr, table_info_->oid_);
    RID new_rid = new_rid_optional.value();
    for (auto idx_info : indexes) {
      idx_info->index_->DeleteEntry(
          old_tuple.KeyFromTuple(table_info_->schema_, idx_info->key_schema_,
                                idx_info->index_->GetKeyAttrs())
          , *rid, nullptr
      );
      idx_info->index_->InsertEntry(
          new_tuple.KeyFromTuple(table_info_->schema_, idx_info->key_schema_, idx_info->index_->GetKeyAttrs()), new_rid, nullptr
      );
    }
    ++update_num;
  }
