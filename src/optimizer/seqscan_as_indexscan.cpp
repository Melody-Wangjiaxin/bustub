...
auto Optimizer::OptimizeSeqScanAsIndexScan(const bustub::AbstractPlanNodeRef &plan) -> AbstractPlanNodeRef {
  ...
  if(optimized_plan->GetType() == PlanType::SeqScan) {
    ...
    if (const auto *cmp_expr = dynamic_cast<const ComparisonExpression*>(seq_scan_plan.filter_predicate_.get());cmp_expr != nullptr) {
      if(cmp_expr->comp_type_ == ComparisonType::Equal) {
        if(const auto *column_value_expr = dynamic_cast<const ColumnValueExpression*>(cmp_expr->children_[0].get());column_value_expr != nullptr) {
