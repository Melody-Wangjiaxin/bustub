...
auto LRUKReplacer::Evict() -> std::optional<frame_id_t> {
	...
	size_t max_timestamp = std::numeric_limits<size_t>::max();
	{
		...
		for(auto &ptr : node_store_) {
			if(!ptr.second.IsEvictable()) continue;
			if(have_less_k_access) {
				if(ptr.second.GetAccessNum() >= k_) continue;
				if(ptr.second.GetHistory() < max_timestamp) {
					max_timestamp = ptr.second.GetHistory();
					fid = ptr.first;
				}
			} else {
				if(ptr.second.GetAccessNum() < k_) {
...
void LRUKReplacer::Remove(frame_id_t frame_id) {
	std::lock_guard<std::mutex> lock(latch_);
	auto ptr = node_store_.find(frame_id);
	if(ptr == node_store_.end()) return;
	if(!(ptr->second.IsEvictable())) throw Exception("Can't find the frame id!");
