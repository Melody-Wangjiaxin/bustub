...
auto BufferPoolManager::NewPage(page_id_t *page_id) -> Page * { 
  ...
  if(!(free_list_.empty())) {
    ...
    std::optional<frame_id_t> op_fid = replacer_->Evict();
    if(op_fid != std::nullopt) {
      f_id = op_fid.value();
      if(pages_[f_id].IsDirty()) {
        auto promise_ = disk_scheduler_->CreatePromise();
        auto future_ = promise_.get_future();
        disk_scheduler_->Schedule(
          {true, pages_[f_id].GetData(), pages_[f_id].page_id_, std::move(promise_)}
        );
        if(!future_.get()) throw Exception("Fail to write disk data!");
      }
...
auto BufferPoolManager::FetchPage(page_id_t page_id, [[maybe_unused]] AccessType access_type) -> Page * {
  ...
  if(ptr == page_table_.end()) {
    ...
    auto future_ = promise_.get_future();
    disk_scheduler_->Schedule(
      {false, pages_[f_id].GetData(), page_id, std::move(promise_)}
    );
    if(!future_.get()) throw Exception("Fail to write disk data!");
  } else {
    f_id = ptr->second;
...
auto BufferPoolManager::FlushPage(page_id_t page_id) -> bool { 
  ...
  auto future_ = promise_.get_future();
  disk_scheduler_->Schedule(
    {true, pages_[f_id].GetData(), page_id, std::move(promise_)}
  );
  if(!future_.get()) throw Exception("Fail to write disk data!");
  pages_[f_id].is_dirty_ = false;
  return true;
...
void BufferPoolManager::FlushAllPages() {
  std::lock_guard<std::mutex> lock(latch_);
  for(auto &p : page_table_) FlushPage(p.first);
}
...
auto BufferPoolManager::DeletePage(page_id_t page_id) -> bool { 
  ...
  if(pages_[ptr->second].pin_count_ > 0) return false;
...
auto BufferPoolManager::FetchPageBasic(page_id_t page_id) -> BasicPageGuard { 
  ...
  if(!p) throw Exception("Fail to fetch page!");
  return {this, p}; 
}
...
auto BufferPoolManager::NewPageGuarded(page_id_t *page_id) -> BasicPageGuard { 
  ...
  if(!p) throw Exception("Fail to new page!");
  return {this, p};
}
